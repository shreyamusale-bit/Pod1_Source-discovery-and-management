...
Platform prototype — FastAPI + SQLite backend with a built-in web dashboard.
Covers the Week 1/Week 2 task: source data model, DISCOVERED → VALIDATED →
AUTHORIZED → ACTIVE lifecycle, authorization governance
(PENDING/APPROVED/REJECTED/EXPIRED), health monitoring, and a full CRUD API.
...
Then open **http://127.0.0.1:8000/** for the web dashboard — add candidates,
filter by lifecycle/authorization/transport, approve or reject pending
sources (with a required "decided by" + "basis" recorded on the row), walk
sources through their lifecycle, quarantine or disable, and see per-source
health at a glance.

Swagger docs for the underlying API are still at
http://127.0.0.1:8000/docs if you want to call it directly or wire up the
crawler side.
...
- **Toolbar** — filter the table by lifecycle, authorization status, or
  transport; "+ New candidate" opens the intake form.
- **Table** — one row per source, with badges for both state machines,
  a rolled-up health indicator, and context-sensitive action buttons
  (only the transitions valid from that row's current state are shown).
- **Approve / Reject dialog** — requires *decided by* and *basis* before
  it will submit, mirroring the registry's audit requirement at the API
  level.
- **Delete vs. Disable** — Delete is a hard delete (confirmation required,
  meant for cleaning up test rows). To revoke a real source, use the
  lifecycle action "Disable" instead — see the note below.
